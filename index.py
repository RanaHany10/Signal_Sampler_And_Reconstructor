...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.reset_button.clicked.connect(self.reset_plot) # Connect the reset button to the reset_plot method
    ...
    def get_signal(self):
        self.signalName = self.signal_combobox.currentText().split(" (")[0]
    ...
    def display_sum(self):
        ...
        # self.composed_signal_xdata = []  # Clear composed_signal_xdata
        
        if len(self.composed_signal_xdata) < len(self.signalSum):
            self.composed_signal_xdata.extend(list(self.time[len(self.composed_signal_xdata):]))
            #(this to ensure that (which becomes self.yData in signalConfirm) always have the same length of self.composed_signal_xdata
        self.signal_plot(self.canvas2, self.widget2,
    ...
    def signalConfirm(self):
        ...
        self.tabWidget.setCurrentIndex(1) #this to move you to the tab2 when press confirm
        self.canvas3.axes.autoscale()
        self.canvas3.axes.plot(self.xData_composed, self.yData_composed)
        self.canvas3.draw()
        self.widget3.setLayout(self.layout3)  #i should add this line to fix problem of not adding signal to canvas3

        self.plot_in_widget3 = True  # set attribute to True after drawing plot

    def load_signal(self):
        self.fname1, _ = QFileDialog.getOpenFileName(self, 'Open file', './', 'CSV Files (*.csv);;Text Files (*.txt)')
        ...
        self.reset_all()
        self.fname_1 = True
        # Clear or reset other variables and data structures
        ...
        if len(data1.columns) == 3:  # Check if there are at least three columns
            ...
            self.max_frequency = data1.iloc[:, 2][:1000].max()  # this to get max freq from 3rd col
        else:
            self.time_col = data1.iloc[:, 0][:1000]
            self.values = data1.iloc[:, 1][:1000]
            f_sample = []
            for i in range(500):
                delta_t = data1.iloc[i + 1, 0] - data1.iloc[i, 0]
                f_sample.append(1/delta_t)
            self.max_frequency = max(f_sample) / 2  # this to get max freq from 3rd col
            print(self.max_frequency)

        # else:
        #     y_values = data1.iloc[:, 0]   # Convert from uV to mV and take only first 1000 values  # y-values are the only column
        #     self.values = y_values[:1000]  # take only the first 1000 values
        #     time_values = np.linspace(0, 20, len(y_values))  # Generate x-values for a 20-second interval
        #     self.time_col = time_values[:1000]  # take only the first 1000 values
        #     delta_t = (self.time_col[1] - self.time_col[0])
        #     print(self.time_col[1])
        #     print(self.time_col[0])
        #     f_Sample = 1 / delta_t
        #     self.max_frequency = f_Sample / 2
    ...
    def sample_signal(self):
        ...
        interp_func = interpolate.interp1d(xData, yData, kind='slinear', fill_value="extrapolate")
        ...
        self.canvas3.axes.scatter(sampled_time_points, self.sampled_values_points, marker='x', color='r')
    ...
    def whittaker_shannon_interpolation(self):
        ...
        interpolated_time_points = np.arange(xData[0], xData[-1],
                                             1/self.sampling_freq)

        # Initialize an array for the reconstructed signal
        self.reconstructed_signal = np.zeros(len(xData))

        # Perform the Whittaker-Shannon interpolation
        # for i, n in enumerate(interpolated_time_points):
        #     self.reconstructed_signal += self.sampled_values_points[i] * np.sinc(self.sampling_freq * (xData - interpolated_time_points[i]))
        for i, n in enumerate(interpolated_time_points):
            for j in range (len(xData)):
                self.reconstructed_signal[j] += self.sampled_values_points[i] * np.sinc(self.sampling_freq * (xData[j] - interpolated_time_points[i]))
    ...
    def plot_error(self):
        ...
        error = np.array(yData) - self.reconstructed_signal

        # Plot the error
        self.canvas5.axes.clear()
        if self.fname_1:
            # self.canvas5.axes.set_ylim(-0.1, 0.2)
    ...
    def add_noise(self):
        if self.plot_in_widget3:  # check if there's a plot in widget 3
            ...
            self.snr_lcd.show()
            self.snr_label.show()
            self.snr_slider.setValue(self.snr_slider.maximum())  # set value to maximum after showing slider
    ...
    def reset_noise(self):
        ...
        self.snr_lcd.hide()
        self.snr_label.hide()
        self.snr_slider.setValue(self.snr_slider.maximum())  # reset slider to maximum value
        snr = self.snr_slider.value()  # get inverted SNR from slider
        self.snr_lcd.display(snr)  # display SNR on LCD
        try:
            self.snr_slider.valueChanged.disconnect(self.apply_noise)  # disconnect slider from apply_noise function
        except TypeError:
